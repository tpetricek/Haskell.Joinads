...
> -- Haskell parser.
> --
> -- Modified by Tomas Petricek to translate the 'docase' syntax as 
> -- defined in: http://www.cl.cam.ac.uk/~tp322/papers/docase.html 
> -- (Implementation is based on 'arrowp' preprocessor by Ross Paterson)
> -- 
> -----------------------------------------------------------------------------
>
> module Parser (
...
>
> import JoinadsSyn -- added for joinads
> }
...
>	'safe'		{ KW_Safe }
>	'unsafe'	{ KW_Unsafe }
> 'docase'  { KW_Docase }     -- added for joinads
...
>	| '-' fexp			{ HsNegApp $2 }
>	| 'do' stmtlist			{ HsDo $2 }
> | srcloc 'docase' '(' texps ')' 'of' altslistJ { translateDoCase $1 (reverse $4) $7 } -- added for joinads
> | srcloc 'docase' exp 'of' altslistJ { translateDoCase $1 [$3] $5 } -- added for joinads
>	| fexp				{ $1 }
...
-----------------------------------------------------------------------------
DoCase alternatives (added for Joinads)

> altslistJ :: { [HsJoinadClause] }
>	: '{'  altsJ '}'			{ $2 }
>	| open altsJ close		{ $2 }

> altsJ :: { [HsJoinadClause] }
>	: optsemis alts1J optsemis	{ reverse $2 }

> alts1J :: { [HsJoinadClause] }
>	: alts1J semis altJ		{ $3 : $1 }
>	| altJ				{ [$1] }

> altJ :: { HsJoinadClause }
> : srcloc '(' patsJ ')' ralt optwhere	  { JoinadClause $1 $3 $5 $6 }
> | srcloc patJ ralt optwhere	            { JoinadClause $1 [$2] $3 $4 }
...
> patJ :: { Maybe HsPat }
>	: exp0b				  {% liftM Just (checkPattern $1) }
>	| '?'				    { Nothing }
